...
namespace Day3
{
    class Program
    {
        ...
        private static int CalculateManhattanDistance(Point point)
        {
            return Math.Abs(point.X) + Math.Abs(point.Y);
        }

        private static long GetNumberOfSteps(List<Move> moves, Point point)
        {
            return moves.Where(move => move.Point.X == point.X && move.Point.Y == point.Y).Sum(move => move.Steps);
        }

        private static long CalculateMinNumberOfSteps(List<Move> moves)
        {
