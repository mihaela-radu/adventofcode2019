...
namespace Day3
{
    class Program
    {
        ...
        {
            var wire1Coordinates = CalculateWireCoordinates(testwire1);
            var wire2Coordinates = CalculateWireCoordinates(testwire2);

            var commonCoordinates = new List<Point>();//wire1Coordinates.Intersect(wire2Coordinates);//wire1Coordinates.Where(coordinate => wire2Coordinates.Contains(coordinate));
            foreach (var wire1Coord in wire1Coordinates)
                foreach (var wire2Coord in wire2Coordinates)
                    if (wire1Coord.X == wire2Coord.X && wire1Coord.Y == wire2Coord.Y)
                        commonCoordinates.Add(wire1Coord);
            
            Console.WriteLine("Hello World!");
        }

        public static string testwire1 = "R8,U5,L5,D3";
        public static string testwire2 = "U7,R6,D4,L4";
        ...
        private static List<Point> CalculateWireCoordinates(string wirePath)
        {
            var wireMoves = wirePath.Split(',');
            var wireCoordinates = new List<Point>();
            var moveIndex = 0;

            foreach(var move in wireMoves)
            {

                var moveX = moveIndex == 0 ? 0 : wireCoordinates[moveIndex -1].X;
                var moveY = moveIndex == 0 ? 0 : wireCoordinates[moveIndex -1].Y;

                var direction = move.Substring(0, 1);
                var movingDistance = int.Parse(move.Substring(1));
                switch (move.Substring(0,1))
                {
                    case "R":
                        moveX += movingDistance;
                        break;
                    case "L":
                        moveX -= movingDistance;
                        break;
                    case "U":
                        moveY += movingDistance;
                        break;
                    case "D":
                        moveY -= movingDistance;
                        break;
                }
                wireCoordinates.Add(new Point(moveX, moveY));
                moveIndex++;
            }
